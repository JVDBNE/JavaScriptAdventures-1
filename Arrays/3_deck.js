...
function shuffle(arr) {
    ...
    for (i = arr.length - 1; i > 0; i--) {
        j = Math.floor(Math.random() * (i + 1));                            //fisher yates shuffle algorythm 
        x = arr[i];
        arr[i] = arr[j];
        arr[j] = x;
    }
  ...
  */
  ...
  function createDeck() {
    ...
    for (let i=0; i<suites.length; i++) {                                 // runs once for each suit
        for (let j=0; j<ranks.length; j++) {                              // runs 13 times 
            let card = [];
